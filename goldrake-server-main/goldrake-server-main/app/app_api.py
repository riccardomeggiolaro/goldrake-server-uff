...
def mainprg():
	...
	async def Weighing(node: Optional[str] = None, id: Optional[int] = None):
		...
		result, status_command, command_executed = weigher.setModope(node=node, modope="WEIGHING", data_assigned=data)
		if result:
			data_in_execution = weigher.deleteDataInExecution(node=node)
			return {
				"result": result,
				"status_command": status_command,
				"data_in_execution": data_in_execution
			}
		else:
			raise HTTPException(status_code=404, detail='Not found')
	...
	async def AddConfigWeigherNode(node: weigher.SetupWeigher):
	...
	async def SetConfigWeigherSetup(node: Optional[str] = None, setup: weigher.SetupWeigherOptional = {}):
		if node == "null":
				node = None
	...
	async def SetConfigWeigherConnection(connection: Union[weigher.SerialPort, weigher.Tcp]):
...
def init():
	...
	configuration = weigher.Configuration(**lb_config.g_config["app_api"]["weigher"])
